<script>
  ...
  $: sentenceCount = text.trim().length
    ? (
        text.match(
          /[^.!?\s][^.!?]*(?:[.!?](?!['"]?\s|$)[^.!?]*)*[.!?]?['"]?(?=\s|$)/g,
        ) || []
    ...
    : 0;
  $: paragraphCount = text.trim().length
    ? text
        .replace(/\r\n/g, "\n")
        .split(/\n{2,}/)
...
<div class="bg-white rounded-2xl shadow-lg p-6 mb-8 transition-colors">
  ...
  <textarea
    class="w-full border border-gray-300 rounded-lg px-4 py-3 text-base md:text-lg lg:text-xl bg-white text-gray-900 resize-y focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
    rows="8"
    bind:value={text}
    placeholder="Paste or type your text here..."
  ></textarea>
  <div
    class="flex flex-wrap gap-8 mt-4 justify-start text-base font-medium text-gray-700"
  >
    <div>
